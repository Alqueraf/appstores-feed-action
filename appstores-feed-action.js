...
appstoresService.getLatestAppsData(playstoreIds, appstoreIds).then(async appsData => {
    if (appsData.length === 0) {
        ...
        try {
            ...
            const appstoresFeedMarkdown = buildAppstoresFeedMarkdown(appsData);
            core.info("Building updated README");
            const newReadme = buildReadme(readmeData, appstoresFeedMarkdown);
...
/**
 * Converts a list of apps data to Markdown
...
const buildAppstoresFeedMarkdown = (appsData) => {
    // Prepare HTML block
    const htmlStartElement = `<div class="grid-container">`;
